...
class JSONStore {
  ...
  // Used when articles are marked read — see the matching comment in
  // db-supabase.js. In-memory, so a simple loop is already cheap here.
  async cacheDeleteByUrls(urls) {
    const data = this._loadCache();
    let changed = false;
    for (const url of urls) {
      if (url in data) { delete data[url]; changed = true; }
    }
    if (changed) this._flushCache();
  }
