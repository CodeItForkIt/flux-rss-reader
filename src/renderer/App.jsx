...
import * as articleState from '../core/article-state.mjs';
...
const GLOBAL_CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .reader-body img:not([width]):not([height]) { aspect-ratio: 16 / 9; object-fit: contain; }
...
// ─── Auth Screen ──────────────────────────────────────────────────────────────
// ─── Sidebar ──────────────────────────────────────────────────────────────────
function Sidebar({ folders, feeds, articles, activeView, onSelectView, onOpenAddFeed, onAddToFolder, onRefreshAll, onExportOPML, onImportOPML, onNewFolder, onOpenSettings, onFeedSettings, onRemoveFeed, onRemoveFolder, onManageFolderFeeds, newArticleCount, settings, onReorderFolders, onEditFolder, refreshing }) {
    ...
    <aside style={{ width:collapsed?52:216, minWidth:collapsed?52:216, background:T.surface, borderRight:`1px solid ${T.border}`, display:'flex', flexDirection:'column', transition:'width 0.2s ease, min-width 0.2s ease', overflow:'hidden' }}>
        ...
        <div style={{ padding:10, borderTop:`1px solid ${T.borderSubtle}`, display:'flex', flexDirection:'column', gap:6 }}>
          ...
          <div style={{ display:'flex', gap:4 }}>
            ...
            <div style={{ position:'relative', display:'inline-flex' }}>
              <IconBtn icon={refreshing?<Spinner size={14} color={T.textMuted}/>:"↺"} title={newArticleCount>0?`${newArticleCount} new article${newArticleCount>1?'s':''} — click to refresh`:'Refresh all'} onClick={onRefreshAll} size={28} />
      ...
      {collapsed&&(
        <div style={{ padding:'8px 6px', borderTop:`1px solid ${T.borderSubtle}`, display:'flex', flexDirection:'column', gap:2 }}>
          ...
          <IconBtn icon={refreshing?<Spinner size={16} color={T.textMuted}/>:"↺"} title="Refresh" onClick={onRefreshAll} size={40} />
...
// Prefetches full article content for the newest unread articles right
// after a refresh, so opening one of them loads instantly instead of
// waiting on the same fetch-and-extract work loadArticleContent would
// otherwise do lazily on click. Deliberately called without being
// awaited by refreshFeeds — this runs in the background after the UI is
// already interactive, and a slow or failed prefetch here should never
// delay or break the refresh itself (a failure just means the article
// fetches fresh on open, exactly as if this never ran — see
// loadArticleContent). This is client-driven specifically because it
// needs to be: a server-side "fire and forget more work after
// responding" approach isn't reliable on Vercel's serverless functions,
// which can freeze shortly after a response is sent with no guarantee
// background work continues. Each prefetch here is instead its own
// complete, properly-awaited request — the same reliability any other
// API call in this app already depends on. Bounded to a modest number
// of the newest unread items (not the whole backlog), since each one is
// a real fetch-the-page-and-run-Readability operation server-side, not
// a cheap one; and skips YouTube items, which never go through
// loadArticleContent's fetch path in the first place.
async function prefetchArticleContent(articles, limit = 12, concurrency = 3) {
  const targets = articles.filter(a => !a.isRead && !a.isYoutube).slice(0, limit);
...
function ArticleRow({ article, feed, folder, isSelected, onClick, deArrowEnabled, settings }) {
  ...
  const largeThumb = showThumb && <div style={{ width:'100%', maxHeight:140, aspectRatio:'16/9', borderRadius:6, overflow:'hidden', background:T.bg, marginBottom:8, position:'relative' }}>
    <img src={displayThumb} alt="" style={{ width:'100%', height:'100%', objectFit:'contain' }} onError={()=>setImgFailed(true)} onLoad={onImgLoad} />
...
function GroupRow({ group, feeds, isSelected, onClick }) {
  ...
  return <div onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{ padding:'11px 13px', borderBottom:`1px solid ${T.borderSubtle}`, cursor:'pointer', background:isSelected?T.surfaceActive:h?T.surfaceHover:'transparent', borderLeft:`3px solid ${isSelected?T.accent:'transparent'}`, transition:'background 0.1s', position:'relative' }}>
    {showThumb&&<div style={{ width:'100%', maxHeight:140, aspectRatio:'16/9', borderRadius:6, overflow:'hidden', background:T.bg, marginBottom:8, position:'relative' }}>
      <img src={top.thumbnail} alt="" style={{ width:'100%', height:'100%', objectFit:'contain' }} onError={()=>setImgFailed(true)} onLoad={onImgLoad} />
    </div>}
...
function ReaderPane({ article, feed, allArticles, allFeeds, onNavigate, onMarkRead, onToggleStar, onOpenRules, onSaveRule, onSaveSettings, settings, isMobile }) {
  ...
  useEffect(()=>{
    if (!article || article.isGroup) return;
    const id = article.id, feedId = article.feedId, link = article.link;
  ...
  return (
    <div ref={outerDivRef} style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', position:'relative' }}>
      ...
      <div style={{ padding:'10px 20px', borderBottom:`1px solid ${T.border}`, display:'flex', alignItems:'center', gap:6, flexShrink:0, background:T.surface }}>
        ...
        <IconBtn icon={article.isRead?'○':'●'} title={article.isRead?'Mark unread':'Mark read'} onClick={()=>{ suppressAutoMarkRef.current = true; onMarkRead(article.id,article.feedId,!article.isRead,article.link); }} size={28} />
      ...
      {isMobile && mobileMoreMenu && (
        <ContextMenu x={mobileMoreMenu.x} y={mobileMoreMenu.y} onClose={()=>setMobileMoreMenu(null)} items={[
          ...
          { label: article.isRead?'Mark unread':'Mark read', icon: article.isRead?'○':'●', onClick:()=>{ suppressAutoMarkRef.current = true; onMarkRead(article.id,article.feedId,!article.isRead,article.link); } },
...
function SettingsModal({ settings, onSave, onClose }) {
  ...
  const [cacheDays, setCacheDays] = useState(settings.articleCacheDays ?? 2);
...
export default function App() {
  ...
  const refreshFeeds = useCallback(async(feedList, state, settingsOverride, force=false)=>{
    ...
    try {
      if (api.isElectron) {
        ...
        const mergeInto = (existing, incoming) => articleState.mergeArticlesWithState(existing, incoming, readSet, starSet);
      ...
      setRefreshing(false);
      if (finalArticlesForCache) prefetchArticleContent(finalArticlesForCache);
  ...
  const handleMarkRead = useCallback(async(articleId, feedId, read=true, url)=>{
    setArticles(prev=>prev.map(a=>a.id===articleId?{...a,isRead:read}:a));
    await persistWithRetry(() => api.articles.markRead({articleId, feedId, read, url}), 'read state');
  },[persistWithRetry]);
  ...
  const handleMarkAllRead = useCallback(async()=>{
    const idsToMark = visibleArticles.filter(a=>!a.isRead).map(a=>({id:a.id, feedId:a.feedId, link:a.link}));
    ...
    const items = idsToMark.map(({id,feedId,link})=>({articleId:id, feedId, url:link}));
