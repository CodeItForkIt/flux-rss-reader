...
// Keyed per userId — see the matching, longer comment in server/index.js.
// This was the same single object shared across every user this instance
// serves, keyed only by domain, silently reusing one user's cookies
// (including any login session established through the inline browser)
// for every other user's requests to that same domain.
...
async function articleTtlMs(userId) {
  ...
  const days = userId != null ? ((await db.getSettings(userId)).articleCacheDays ?? 2) : 2;
...
// `true` (trust every hop in X-Forwarded-For) let anyone bypass every
// IP-keyed rate limiter below -- most importantly authLimiter, guarding
// login/2FA -- just by sending their own fake X-Forwarded-For header, since
// with every hop trusted there's no way to distinguish a real proxy's
// value from an attacker's. Unlike server/index.js (always behind exactly
// Vercel's edge, so always exactly 1 real hop), this self-hosted server's
// topology depends on how it's deployed -- direct-exposed, or behind a
// reverse proxy the operator controls -- so it isn't safe to guess a hop
// count here. Defaults to trusting none (accurate whenever there's no
// proxy in front, which is the more common self-hosted case); set
// TRUST_PROXY_HOPS to the number of real reverse-proxy hops in front of
// this server if you're running one.
...
// /proxy is not special-cased — see the matching comment in
// server/index.js for why that bypass was unnecessary and unsafe.
...
app.post('/api/feeds/fetch-all', async (req, res) => {
  ...
  await Promise.all(Array.from({ length: CONCURRENCY }, async () => {
    while (next < feeds.length) {
      const i = next++;
      try { results[i] = { ok: true, ...await fetchFeed(feeds[i], userCookieJars(req.userId)) }; }
...
app.post('/api/feeds/:id/fetch', async (req, res) => {
  ...
  if (!feed) return res.status(404).json({ error: 'Not found' });
  try { res.json({ ok: true, ...await fetchFeed(feedRow(feed), userCookieJars(req.userId)) }); }
...
app.post('/api/articles/mark-read', async (req, res) => {
  const { articleId, feedId, url } = req.body;
  await db.markRead(req.userId, `${feedId}:${articleId}`);
  if (url) {
    try { await articleCache.delete(url); } catch {}
  }
  res.json({ ok: true });
});
...
app.get('/api/proxy', async (req, res) => {
  ...
  try {
    const upstream = await fetchWithCookies(target, {}, userCookieJars(req.userId));
    ...
    if (ct.includes('text/html')) {
      ...
      html = html
        ...
        // this document is served from Flux's own origin instead.
